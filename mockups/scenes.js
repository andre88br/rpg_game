...
import { text, textWidth, wrap } from './font.js';
import * as T from './tiles.js';
import { CAST } from './people.js';
import * as CR from './creatures.js';
import * as UI from './ui.js';
import { medalha, fileiraMedalhas, MEDALHAS } from './badges.js';

export const W = 240, H = 160, TS = 16;

/* ---------- cenario base de PORTO IARA (15 x 10 tiles) ---------- */
export function mapaPortoIara() {
  const b = new Buf(W, H);
  const put = (tile, tx, ty) => b.blit(tile, tx * TS, ty * TS);

  // grama em tudo, com variacao por tile para nao ficar repetitivo
  for (let ty = 0; ty < 10; ty++)
    for (let tx = 0; tx < 15; tx++) put(T.tileGrass(tx * 31 + ty * 17 + 3), tx, ty);

  // agua nas duas ultimas fileiras + faixa de areia
  for (let tx = 0; tx < 15; tx++) {
    put(T.tileShore(tx * 5 + 2), tx, 8);
    put(T.tileWater(tx * 13 + 7), tx, 9);
  }
  // recorte de areia/agua: a agua sobe em degrau no meio
  for (let tx = 9; tx < 15; tx++) put(T.tileWater(tx * 9 + 1), tx, 8);

  // caminho de terra: desce do topo e vira para o ginasio
  for (let ty = 0; ty < 6; ty++) put(T.tilePath(ty * 7 + 1), 7, ty);
  for (let tx = 3; tx < 12; tx++) put(T.tilePath(tx * 3 + 9), tx, 4);

  // arvores nas bordas
  for (let tx = 0; tx < 15; tx++) if (tx !== 7) put(T.tileTree(tx * 11), tx, 0);
  for (const [tx, ty] of [[0, 1], [0, 2], [14, 1], [14, 2], [0, 6], [14, 6], [13, 2]])
    put(T.tileTree(tx * 7 + ty * 3), tx, ty);

  // mato alto: onde aparecem os Encantados selvagens
  for (const [tx, ty] of [[1, 5], [2, 5], [3, 5], [1, 6], [2, 6], [3, 6], [2, 7], [1, 7]])
    put(T.tileTallGrass(tx * 5 + ty * 13), tx, ty);

  // flores e pedras decorativas
  put(T.tileFlower(3, '#f2d24b'), 5, 2);
  put(T.tileFlower(9, '#e07ab0'), 11, 6);
  put(T.tileRock(4), 12, 5);

  // casa do pescador e mercado
  const casa = T.building(3, 2, { roof: P.roof, roofD: P.roofD, roofL: P.roofL });
  b.blit(casa, 2 * TS, 1 * TS + 4);
  const loja = T.building(3, 2, { roof: '#3f8f6f', roofD: '#2b6b52', roofL: '#5fb894' });
  b.blit(loja, 10 * TS, 1 * TS + 4);

  // GINASIO DE PORTO IARA: maior, telhado azul, fica de frente para o mar
  const gin = T.building(4, 3, {
...
export function telaMundo(opt = {}) {
  ...
  if (guarda) {
    ...
    b.blit(CAST.guarda(), 5 * TS + 24, 5 * TS + 26);
  }

  // pescador perto do pier
  b.blit(CAST.pescador(), 9 * TS + 4, 7 * TS + 6);
  // NPC de rua
  b.blit(CAST.firmina(), 11 * TS + 2, 3 * TS + 8);

  // jogador no caminho, indo em direcao ao ginasio
  b.blit(CAST.taina(), 7 * TS + 4, 4 * TS + 2);

  // moitas soltas e placa
  b.blit(T.signpost(), 4 * TS + 2, 4 * TS - 14);

  // faixa com o nome da cidade, como aparece ao entrar
  if (banner) {
    const nome = 'PORTO IARA';
    const cx = UI.caixa(textWidth(nome) + 20, 20);
    text(cx, nome, 10, 6, P.uiInk);
    b.blit(cx, 6, 6);
...
export function telaDialogo() {
  ...
  b.blit(CAST.pescador(), 9 * TS + 4, 4 * TS + 2);
  b.blit(CAST.taina(), 11 * TS, 4 * TS + 2);

  const linhas = wrap('Sem as redes ninguém pesca hoje, moça. Dizem que um Encantado levou as três.', 200);
  ...
  text(t, 'TAREFA', 6, 5, P.uiAccD);
  text(t, '1/3', tw - 9 - textWidth('1/3'), 5, P.uiInk);
  text(t, 'REDES PERDIDAS', 6, 15, P.uiInk);
...
function plataforma(b, cx, cy, rx, ry) {
...
export function telaBatalha(opt = {}) {
  ...
  if (modo === 'comando') {
    ...
    const linhas = wrap('O que IARINHA vai fazer?', 128);
    linhas.forEach((l, i) => text(cx, l, 8, 10 + i * 11, P.uiInk));
    b.blit(cx, 0, H - 50);
    ...
    ficha.blit(UI.etiquetaTipo('ÁGUA', TYPES.agua.cor, TYPES.agua.corD), 6, 5);
    text(ficha, 'PP  15/15', 6, 20, P.uiInk);
    text(ficha, 'POT 40', 6, 30, P.uiInk);
    b.blit(ficha, W - 78, H - 50);
    // aviso de eficacia
    const av = UI.caixa(104, 16, { fundo: P.uiAcc });
    text(av, 'É SUPER EFICAZ!', 6, 4, P.uiInk);
    b.blit(av, 8, H - 68);
  }
  return b;
}
...
export function telaTitulo() {
  ...
  text(t1, 'ENCANTADOS', 0, 1, P.gold);
  const logo = escalar(t1, 3);
  logo.outline(P.ink);
  b.blit(logo, (W - logo.w) / 2, 26);

  const t2 = new Buf(30 * 6, 9);
  text(t2, 'A TRILHA DAS OITO MEDALHAS', 0, 1, '#f0e6c8');
  const sub = escalar(t2, 1);
  b.blit(sub, (W - textWidth('A TRILHA DAS OITO MEDALHAS')) / 2, 62);
  ...
  const mx = (W - textWidth(msg)) / 2;
  text(b, msg, mx, H - 26, P.white, { shadow: P.ink });
  // botao A desenhado no meio da frase
  const bx = mx + 6 * 6;
  b.circle(bx + 4, H - 23, 6, P.uiAcc);
  b.circle(bx + 4, H - 23, 5, '#f0d878');
  text(b, 'A', bx + 2, H - 26, P.uiInk);

  const rod = 'ENCANTADOS 2026';
  text(b, rod, (W - textWidth(rod)) / 2, H - 11, '#7f749c');
  return b;
}

function mistura(a, b2, t) {
...
export function telaMapa() {
  ...
  text(b, titulo, (W - textWidth(titulo)) / 2, 12, '#6b4a22');
  ...
  cidades.forEach((c, i) => {
    ...
    const cor = c.t ? TYPES[c.t].cor : '#b0a48c';
    const corD = c.t ? TYPES[c.t].corD : '#7f745f';

    b.circle(x, y, 5, corD); b.circle(x, y, 4, cor); b.set(x - 1, y - 2, P.white);

    text(b, c.n, x + 13, y - 8, '#3a2a14');
    if (c.t) {
      b.blit(UI.etiquetaTipo(TYPES[c.t].nome, cor, corD), x + 15 + textWidth(c.n), y - 10);
      b.blit(medalha(MEDALHAS[i - 1].id, 14), W - 24, y - 7);
      text(b, 'DOM: ' + c.dom, x + 13, y + 2, '#7a5f38');
    } else {
      text(b, c.dom, x + 13, y + 2, '#7a5f38');
    }
  });
  ...
  text(cxT, 'TORNEIO CÍRCULO DOURADO', 8, 6, P.uiInk);
  text(cxT, '6 ADVERSÁRIOS SEGUIDOS', 8, 17, '#6b5210');
  b.blit(cxT, bx, by);
...
function retanguloArredondado(b, x, y, w, h, r, cor) {
...
function botao(b, cx, cy, r, rotulo, cor, corD) {
  b.circle(cx, cy + 2, r, '#0c0a12');       // sombra
  b.circle(cx, cy, r, corD);
  b.circle(cx, cy - 1, r - 2, cor);
  b.ellipse(cx, cy - r / 2, r - 4, 2, '#ffffff22' ? clarearHex(cor) : cor);
  text(b, rotulo, cx - 2, cy - 3, P.ink);
}

function clarearHex(hex) {
  const n = parseInt(hex.slice(1), 16);
  return '#' + [(n >> 16) & 255, (n >> 8) & 255, n & 255]
    .map(v => Math.min(255, v + 45).toString(16).padStart(2, '0')).join('');
}

export function telaCelular() {
  const CW = 268, CH = 486;
  ...
  text(cxInfo, 'TAINÁ', 6, 7, P.uiInk);
  text(cxInfo, 'PORTO IARA', 78, 7, P.uiInk);
  cxInfo.blit(fileiraMedalhas(0, 9, 2), 146, 6);
  b.blit(cxInfo, 14, barraY);

  // ---- controles de toque ----
  const areaY = 250;
  text(b, 'CONTROLES DE TOQUE', (CW - textWidth('CONTROLES DE TOQUE')) / 2, areaY, '#5f5870');

  // direcional: cruz de 3x3 com almofadas de 34px
  const dx = 74, dy = 340, pad = 34;
  const setas = [[0, -1, '^'], [0, 1, 'v'], [-1, 0, '<'], [1, 0, '>']];
  ...
  text(b, 'A = FALAR / CONFIRMAR', 14, 404, '#7f7794');
  text(b, 'B = CORRER / VOLTAR', 14, 416, '#7f7794');

  // botao de menu
  retanguloArredondado(b, CW - 66, 404, 52, 22, 6, '#3a3547');
  text(b, 'MENU', CW - 58, 411, '#cdc6de');
