...
import { P, TIPOS } from './palette.ts';
import type { Tipo } from './palette.ts';

function moldura(b: Buf, _c: string, cD: string): Buf { b.outline(cD); return b; }

export interface Medalha {
  id: string; nome: string; cidade: string;
  tipo: Tipo; lider: string; dom: string;
}

export const MEDALHAS: readonly Medalha[] = [
...
export function medalha(id: string, size = 16): Buf {
...
export function fileiraMedalhas(conquistadas = 3, size = 16, gap = 3): Buf {
